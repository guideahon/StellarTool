...
- Carga mods como `.pak`, `.zip`, carpeta o contenedores **Zen/IoStore** (`.ucas/.utoc`).
...
- Edición manual y masiva de valores (×N, +, −, clamp con regex).
- Import/export de patches TOML (estilo automod).
- Genera un único `zzz_StellarTool_Merged.pak` (formato Zen/IoStore) verificado, listo para `~mods`.
- Convierte outfits **replacer ↔ CNS** desde la sección **CNS Converter**, sin
  ...
  persistente con acceso a la carpeta de salida.
...
- **Stellar Souls Builder** compila un mod personalizado por cambio: daño Beta/Burst,
  drones, dash, ataques de EVE, daño/vulnerabilidad de enemigos, perfect dodge,
  duración de Tachy, economía de gauges, Blaster Cell, outfit, mini-bosses y extras
  independientes. Incluye ajustes de mundo y progresión (tienda, drops, EXP, pesca).
  Presets exportables/importables como `.stpreset`.
- Autoactualización desde GitHub Releases.
- 10 idiomas: es, en, fr, it, de, ja, ko, pt_BR, ru, zh_Hans.
...
incluye el runtime de Qt y los binarios de `tools\` (repak, retoc, UAssetGUI,
cue4parse y el mapping de Stellar Blade). No instala nada ni toca archivos del juego.
...
cd StellarTool
setup.bat                   :: descarga a tools\: repak, retoc, UAssetGUI, cue4parse, usmap y datos CNS
build.bat Release NOPAUSE   :: genera build\Release\StellarTool.exe (con Qt deployado)
tests.bat Release           :: corre la suite de unit tests (diff/merge/CNS/headless)
```
...
1. **Mods**: arrastrá tus mods (.pak/.zip/carpeta/Zen). El orden define prioridad (el primero gana).
2. Opcional: **Importar baseline** — carpeta con JSONs de tablas vanilla (dump de UAssetGUI/CUE4Parse) para ver "antes → después". Sin baseline la tool funciona igual, comparando solo entre mods. También se construye con un clic desde Ajustes.
3. **Analizar cambios** → pestaña **Cambios**: check por cambio, filtro por texto o solo conflictos, todo/nada por tabla. Edición manual y masiva de valores.
...
GunGorgon. Los extras cuantitativos incluyen slider y campo numérico sincronizados;
**Vanilla** desactiva el cambio y restaura sus valores originales. **Selección
avanzada** despliega las propiedades internas agrupadas.

La sección **Mundo y progresión** cubre tablas que ni el pak de combate ni el de
outfit tocan: precios de tienda (`ShopItemTable`), drops de enemigos y cofres
...
`StellarSouls-World`, aparte del de combate.

Las configuraciones completas se pueden guardar como presets con nombre, cargar
y eliminar. Cada preset se **exporta como archivo `.stpreset`** y se puede
**importar**: así se comparte una configuración entera sin publicar un pak.

### CNS Converter

Convierte outfits entre formato replacer y CNS sin modificar el mod de entrada.
Acepta `.zip`, `.pak`, `.utoc` o carpeta. Genera un ZIP instalable con Vortex
y mantiene un historial persistente de conversiones.

### CNS ID Fixer

Escanea recursivamente mods IoStore instalados, detecta y corrige
`Container_Id` duplicados con backup verificable, y reporta conflictos de
`Package_Id` sin modificarlos.
...
- `build` compila un Stellar Souls personalizado desde respuestas JSON o un `.stpreset`.
...
- **Entrada**: paks legacy, zips, carpetas con `.uasset`, **y mods Zen/IoStore**
...
- **Una tabla sin cambios aplicados no se emite**: el pak mergeado carga con
  prioridad máxima, así que empaquetar una copia de vanilla pisaría al mod
  de origen. Se deja fuera y se avisa (`merge_report.txt`).
...
- **Baseline vanilla**: se construye con un clic desde Ajustes (lee todas las tablas
  ...
  **Si tenés mods instalados en `~mods`**: extraé desde una carpeta staging que
  ...
  garantizar baseline vanilla pura.
