...
- **Antes de tocar la escritura de uassets, leer [ARCHITECTURE.md §7](ARCHITECTURE.md)**: UAssetGUI reporta sus errores por el portapapeles (no imprime nada), los FName nuevos deben ir al `NameMap`, el float cero se serializa como el string `"+0"`, el usmap se pasa por ruta y no por nombre. Cada una de esas costó horas de depuración.
- **Mods Zen (`clean=true`)**: se escriben números, texto y enums. Arrays/objetos y filas enteras nuevas/quitadas **no** round-tripean; está deshabilitado a propósito tras medir que no aplicaba ni un cambio de más. No re-habilitarlo sin medir `applied`/`skipped` por tabla contra un mod Zen real, antes y después.
- Al depurar con UAssetGUI: **matar instancias colgadas antes de cada corrida** (una GUI abierta hace fallar entradas válidas) y no confiar en `qWarning`/stderr desde headless — escribir a archivo.
